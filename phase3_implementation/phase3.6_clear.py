...
class RectilinearMotionApp:
    ...
    def setup_ui(self):
        ...
        row += 1
        ttk.Button(control_frame, text="Очистити графік",command=self.clear_plot).grid(row=row, column=0, columnspan=2, pady=5)
        row += 1
        ttk.Button(control_frame, text="За замовчуванням", command=self.reset_defaults).grid(row=row, column=0, columnspan=2, pady=5)
        row += 1
    ...
    """
    Фаза 3.6: Очищення графіка та скидання до значень за замовчуванням
    """
    def clear_plot(self):
        """Очищення графіка"""
        for line in self.ax.lines:
